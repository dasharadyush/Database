...
-- создание таблиц (синтаксис выучим чуть позже). Эти таблицы реально появятся в Вашей БД. 
/*
	Если таблицы не отображаются в дереве объектов, то сверните дерево до уровня базы, нажмите на кнопку обновить на панели Object Explorer (где отображается дерево объектов)
*/
...
-- это старый синтаксис вставки на случай, если у Вас не очень свежая версия. На текущий момент поддерживается более лаконичная форма (её можно было увидеть в скриптах для занятий)
...
/*
	В результате выполнения скрипта у Вас должна появится БД MDZ3 с 4-мя таблицами, в каждой таблице должно появиться несколько строк данных.
	
	Синтаксис для T-SQL, если у Вас другая СУБД, то возможно потребуются незначительные модификации.
	В том числе, если названия покупателей или товаров у Вас не отображаются в читаемом виде, то не переживайте, это не очень существенно.
*/